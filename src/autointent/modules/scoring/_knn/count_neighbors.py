...
def get_counts(labels: NDArray[np.int_], n_classes: int, weights: NDArray[np.float64]) -> NDArray[np.int64]:
    ...
    Args:
        ...
        weights: np.ndarray of shape (n_samples, n_candidates) with integer labels from `[0,n_classes-1]`

    Returns:
        np.ndarray of shape (n_samples, n_classes) with statistics of how many times each class
