...
def multilabel_predict(scores: npt.NDArray[Any], r: float, tags: list[Tag] | None
    ) -> npt.NDArray[Any]:
...
def multilabel_score(y_true: npt.NDArray[Any], y_pred: npt.NDArray[Any]) -> float:
    ...
    in_pred = y_pred[y_true != -1]
    in_true = y_true[y_true != -1]

    return f1_score(in_pred, in_true, average="weighted")  # type: ignore[no-any-return]
