...
class VectorDBModule(RetrievalModule):
    ...
    def dump(self, path: str) -> None:
        ...
        with (dump_dir / "vector_index_client_kwargs.json").open() as file:
            json.dump(self.vector_index_client_kwargs, file)
    ...
    def predict(self, utterances: list[str]) -> list[int | list[int]]:
        """
        return labels and texts of retrieved nearest neighbors
        """
        labels_pred, _, texts = self.vector_index.query(
            utterances,
            self.k,
        )
        return labels_pred, texts
