...
# Warning: This file is temporary here until ether the main project accepts our modifications, or we release a fork
...
CATEGORIES = {
    ...
    sre_parse.CATEGORY_WORD: [unichr(x) for x in range(256) if match(r"\w", unichr(x), U)],
    sre_parse.CATEGORY_NOT_WORD: [unichr(x) for x in range(256) if match(r"\W", unichr(x), U)],
    ...
    for i in d:
        if i[0] == sre_parse.IN:
            ...
            if len(subexpr) == 0:
                ...
                if count:
                    strings = (strings or 1) * (
                        sum(ggen([0], _gen, subexpr, limit=limit, count=True, grouprefs=grouprefs)) or 1
                    )
...
def argparser():
    ...
    argp.add_argument(
        "-r", "--random", help="Returns a random string that matches to the regex", default=False, action="store_true"
    )
    ...
    argp.add_argument("--seed", help="RNG seed", default=None, type=int)
